...
const positionVariants = cva(cn("data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out", "data-[state=closed]:fade-out-80"), {
  variants: {
    position: {
      ...
      "top-middle": "data-[state=closed]:slide-out-to-top-full data-[state=open]:slide-in-from-top-full",
    },
  },
  defaultVariants: {
...
export type PositionVariant = VariantProps<typeof positionVariants>;

const Toast = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Root>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Root> & VariantProps<typeof toastVariants> & PositionVariant>(
  ({ className, variant, position, ...props }, ref) => {
    return <ToastPrimitives.Root ref={ref} className={cn(toastVariants({ variant }), positionVariants({ position }), className)} {...props} />;
  },
);
