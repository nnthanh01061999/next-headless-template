...
import { PositionVariant, Toast, ToastClose, ToastDescription, ToastProvider, ToastTitle, ToastViewport } from "@/components/ui/toast";
...
const toastViewportPositionClasses = {
  ...
  "top-middle": "sm:bottom-auto sm:right-1/2 sm:translate-x-1/2 sm:top-0 sm:flex-col-reverse",
  ...
  return toastsGroupPosition.map((group) => {
    return (
      <ToastProvider key={group.position}>
        {group.toasts?.map(function ({ id, title, description, action, ...props }) {
          return (
            <Toast key={id} {...props}>
        ...
        <ToastViewport className={toastViewportPositionClasses[group.position as keyof typeof toastViewportPositionClasses]} />
      </ToastProvider>
    );
  });
}
